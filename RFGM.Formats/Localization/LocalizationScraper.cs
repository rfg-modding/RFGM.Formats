...
namespace RFGM.Formats.Localization
{
    public static class LocalizationScraper
    {
        ...
        public static void GetIdentifiers(string directoryPath)
        {
                ...
                .Where(file => Path.GetFileName(file) != "credits.xtbl")) // Ignore - gives invalid character exception
            {
                try
                {
                    ...
                    {
                        var hash = HashVolitionCRCAlt(element.Value);
        ...
        internal static uint HashVolitionCRCAlt(string input)
        {
            uint temp = ~0u;
            ...
            foreach (char c in input)
            {
                ...
                temp = temp >> 8 ^ _CrcVolitionTable[(currentChar ^ temp) & 255];
            }

            return ~temp;
        }

        private static readonly uint[] _CrcVolitionTable =
        {
